...
function DonutChart() {
  ...
  function drawChart() {
    const w =
      width === 0
        ? Math.floor(
            Number(window.getComputedStyle(svgRef.current).width.slice(0, -2))
          )
        : width;
    ...
    svg
      ...
      .attr(
        "transform",
        `translate(${Math.floor(w / 2)}, ${Math.floor(h / 2)})`
      )
      ...
      .attr(
        "transform",
        `translate(${Math.floor(w / 2)}, ${Math.floor(h / 2)}) rotate(180)`
      );
  }
  ...
  return (
    <div className="chart__container">
      <div className="charts chart1" ref={svgRef}></div>
    </div>
  );
}
