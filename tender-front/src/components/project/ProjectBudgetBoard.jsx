...
function mapStateToProps(state, { view }) {
  const selectedBudget =
    /* eslint-disable */
    Object.keys(state.project.data).length === 0
      ? ""
      : view === ""
      ? { ...state.project.data[0] }
      : { ...state.project.data.filter((item) => item._id === view)[0] };
  /* eslint-enable */

  function formatSelectedBudget(budget) {
    console.log(
      "ENTERING FORMATTING FUNCTION WITH ",
      budget,
      budget.data.entries
    );
  ...
  function formatIntToCurrency(currentNumber, currency) {
    ...
    const numberWithThousandPoint =
      newNumber.length >= 7
        ? `${newNumber.slice(0, -6)}.${newNumber.slice(-6)}`
        : newNumber;
    const formatedCurrency = currency.toLowerCase();
    let newCurrency;
    switch (formatedCurrency) {
      case "eur":
        newCurrency = "€";
        break;
      case "gbp":
  ...
  const formatedBudget =
    selectedBudget === "" && view === ""
      ? ""
      : formatSelectedBudget(selectedBudget);
