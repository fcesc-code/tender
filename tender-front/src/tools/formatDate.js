...
const formatDate = function formatDate(
  stringDate,
  type = { time: true, date: "long" }
) {
  const date = new Date(stringDate);

  const time = type.time
    ? `${time60Formater(date.getHours())}:${time60Formater(date.getMinutes())}h`
    : "";
  let formatedDate;
  let separator;
  if (type.date === "long" || type.date === undefined) {
    formatedDate = `${getWeekdayName(date.getDay())}, ${getDaySuffix(
      date.getDate()
    )} ${getMonthName(date.getMonth())} ${date.getFullYear()}`;
    separator = type.time ? " on " : "";
  } else if (type.date === "short") {
    formatedDate = `${date.getDate()}/${date.getMonth() + 1}/${String(
      date.getFullYear()
    ).slice(-2)}`;
    separator = type.time ? " " : "";
